...
module.exports = async (req, res) => {
  ...
  const x = parseFloat(req.query.x), y = parseFloat(req.query.y), d = 0.0003;
  const bbox = `${(x-d).toFixed(6)},${(y-d).toFixed(6)},${(x+d).toFixed(6)},${(y+d).toFixed(6)}`;
  const url = `https://api.vworld.kr/req/wfs?service=WFS&version=1.1.0&request=GetFeature&typename=lt_c_uq111&bbox=${bbox}&srsname=EPSG:4326&output=application/json&key=${VWORLD_KEY}`;
  try {
