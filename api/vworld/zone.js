...
module.exports=async(req,res)=>{
  ...
  const x=+req.query.x,y=+req.query.y,d=0.0003;
  const b=`${(x-d).toFixed(6)},${(y-d).toFixed(6)},${(x+d).toFixed(6)},${(y+d).toFixed(6)}`;
  const u=`https://api.vworld.kr/req/wfs?service=WFS&version=1.1.0&request=GetFeature&typename=lt_c_uq111&bbox=${b}&srsname=EPSG:4326&output=application/json&key=${KEY}`;
